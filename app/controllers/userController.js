      ...
      if (req.query.token && req.query.PayerID) {
              ...
              .then(function(user) {
                ...
                if (user.premium && user.premium > currentDate) {
                  currentDate = new Date(
                    user.premium.getTime() + 30 * 24 * 60 * 60 * 1000
                  );
        ...
        if (req.user.premium) {
          res.render("user/edit", {
            ...
            check: req.user.facebookId || req.user.googleId
          });
        } else {
          res.render("user/edit",{
            check: req.user.facebookId || req.user.googleId
  ...
  update: function(req, res, next) {
    try {
      ...
      User.findOne({ where: { email: req.body.email } }).then(function(user) {
        ...
        if (!SaltAndHash.validate(user.hash, user.salt, req.body.oldPassword)) {
